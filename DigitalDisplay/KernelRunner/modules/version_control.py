...
class Repository(object):
    ...
    def init_local_repo(self):
        ...
        try:
            ...
            LOGGER.debug("Error occurred on init_local_repo()")
    ...
    def update_local_repo(self):
        ...
        try:
            ...
            LOGGER.warning("Error occurred on update_local_repo()")
    ...
    def check_remote_changes(self):
        ...
        try:
            ...
            LOGGER.warning("Error occurred on check_remote_changes():local " + str(sys.exc_info()[1]))
            return False
        ...
        try:
            ...
            LOGGER.warning("Error occurred on check_remote_changes():remote " + str(sys.exc_info()[1]))
            return False
...
def update_kernel(kernel_conf, repo_conf, safe_exit_handler):
    ...
    if git.check_remote_changes():
        ...
        copy_kernel()
    lock.release()
    threading.Timer(kernel_utils.get_seconds(dd_kernel_poll_int), update_kernel,
                    [kernel_conf, repo_conf, safe_exit_handler]).start()
...
def update_content(content_conf, repo_conf, safe_exit_handler):
    ...
    if git.check_remote_changes():
        ...
        copy_content()
    lock.release()
    threading.Timer(kernel_utils.get_seconds(dd_content_poll_int), update_content,
                    [content_conf, repo_conf, safe_exit_handler]).start()
